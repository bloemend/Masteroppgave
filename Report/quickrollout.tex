...
\paragraph{Script}
A script is a list of commands that can be executed without user interaction, to automate a process. In order to connect the mesh network to internet a list of commands have to be executed. One idea to speed up the process of setting up the network was to create a script to automate this process. One way to do this could be by creating a self executable script that could be included in the emergency box on for example on an USB-stick. There would have to be a different script, and different USBs, for each up-link type. These scripts would include
\begin{itemize}
